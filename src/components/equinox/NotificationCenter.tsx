...
const CATEGORY_META = {
  ...
  crisis:     { label: 'Crisis',        icon: ShieldAlert, color: 'text-rose-500 dark:text-rose-400 bg-rose-500/10 border-rose-500/30' },
  assignment: { label: 'Assignment',    icon: UserPlus,    color: 'text-sky-600 dark:text-sky-400 bg-sky-500/10 border-sky-500/30' },
  system:     { label: 'System',        icon: Package,     color: 'text-amber-600 dark:text-amber-400 bg-amber-500/10 border-amber-500/30' },
  team:       { label: 'Team',          icon: Users,       color: 'text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/30' },
};
  ...
  return (
    <div className="relative" ref={popoverRef}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="relative h-9 w-9 sm:w-auto sm:px-3 rounded-lg border border-slate-200 bg-slate-100 dark:border-white/10 dark:bg-black/40 hover:bg-slate-200 dark:hover:bg-white/5 text-slate-700 dark:text-neutral-300 flex items-center justify-center gap-2 transition"
      >
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-[340px] sm:w-[400px] z-50 rounded-2xl bg-white border border-slate-200 dark:bg-neutral-950 dark:border-white/10 shadow-2xl overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
            ...
            <button
              ...
              onClick={markAllRead}
              disabled={unread === 0}
              className="text-xs text-slate-500 dark:text-neutral-400 hover:text-primary transition flex items-center gap-1 disabled:opacity-40"
            >
              <CheckCheck className="w-3.5 h-3.5" /> Mark all read
            </button>
          </div>

          <div className="px-3 py-2 border-b border-slate-200 dark:border-white/5 flex items-center gap-1 overflow-x-auto no-scrollbar">
            {CATS.map((c) => (
              <button
                key={c}
                onClick={() => setFilter(c)}
                className={`shrink-0 text-[11px] px-2.5 py-1 rounded-full border transition capitalize ${
                  filter === c
                    ...
                    : 'border-slate-200 dark:border-white/10 text-slate-600 dark:text-neutral-400 hover:border-slate-300 dark:hover:border-white/20'
                }`}
              >
          ...
          <div className="max-h-[360px] overflow-y-auto no-scrollbar divide-y divide-slate-100 dark:divide-white/5">
            {filtered.length === 0 ? (
              <div className="text-xs text-center text-slate-500 dark:text-muted-foreground py-8 italic">No notifications found.</div>
            ) : (
              filtered.map((n) => {
                ...
                return (
                  <button
                    key={n.id}
                    onClick={() => activate(n)}
                    className={`w-full text-left px-4 py-3 flex items-start gap-3 hover:bg-slate-50 dark:hover:bg-white/5 transition ${
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold text-slate-900 dark:text-white truncate">{n.title}</span>
                        {!n.read && <span className="w-1.5 h-1.5 rounded-full bg-primary shrink-0" />}
                      </div>
                      <div className="text-[11px] text-slate-600 dark:text-neutral-400 line-clamp-2 mt-0.5">{n.body}</div>
                      <div className="text-[10px] text-slate-400 dark:text-muted-foreground mt-1 flex items-center gap-2">
