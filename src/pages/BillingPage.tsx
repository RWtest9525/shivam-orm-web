    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredClients.map((c) => {
              ...
              return (
                <div
                  key={c.id}
                  className="bg-white dark:bg-black/40 border border-slate-200 dark:border-white/10 rounded-3xl p-5 sm:p-6 shadow-sm hover:shadow-xl hover:border-primary/40 transition-all flex flex-col justify-between space-y-4 group min-w-0"
                >
                  <div className="space-y-4 min-w-0">
                    {/* Header */}
                    <div className="flex items-start justify-between gap-2.5 min-w-0">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center text-slate-950 font-bold shrink-0 shadow-md overflow-hidden">
                          {c.app_icon_url ? (
                        ...
                        <div className="min-w-0 flex-1">
                          <h3 className="text-sm sm:text-base font-black text-slate-900 dark:text-white group-hover:text-primary transition truncate">
                            {c.company_name}
                          </h3>
                      ...
                      <span className={cn(
                        "px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border shrink-0 whitespace-nowrap",
                        hasPending
                    ...
                    <div className="grid grid-cols-3 gap-1.5 p-3 rounded-2xl bg-slate-50 dark:bg-white/[0.02] border border-slate-200/60 dark:border-white/5 text-center min-w-0">
                      <div className="min-w-0">
                        <div className="text-[10px] uppercase font-bold text-slate-400 truncate">Total</div>
                        <div className="text-xs font-black text-slate-900 dark:text-white mt-0.5 truncate">₹ {bRecord.total_amount.toLocaleString('en-IN')}</div>
                      </div>
                      <div className="min-w-0">
                        <div className="text-[10px] uppercase font-bold text-slate-400 truncate">Paid</div>
                        <div className="text-xs font-bold text-emerald-600 dark:text-emerald-400 mt-0.5 truncate">₹ {bRecord.paid_amount.toLocaleString('en-IN')}</div>
                      </div>
                      <div className="min-w-0">
                        <div className="text-[10px] uppercase font-bold text-slate-400 truncate">Pending</div>
                        <div className={cn("text-xs font-black mt-0.5 truncate", hasPending ? "text-rose-600 dark:text-rose-400" : "text-emerald-500")}>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 text-xs min-w-0">
                      {bRecord.excel_sheet_url ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold hover:bg-emerald-500/20 transition border border-emerald-500/20 text-[11px] cursor-pointer shrink-0"
                        >
                      ...
                      <span className="text-[10px] font-extrabold text-amber-600 dark:text-amber-400 bg-amber-500/10 px-2 py-1 rounded-lg shrink-0">
                  ...
                  {/* Actions Bar */}
                  <div className="pt-3 border-t border-slate-200 dark:border-white/10 flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 min-w-0">
                    {isAdmin && (
                      <button
                        onClick={() => handleOpenEditBilling(bRecord)}
                        className="px-3 py-2 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 font-bold text-xs transition border border-slate-200 dark:border-white/5 shrink-0 whitespace-nowrap"
                      >
                    ...
                    <button
                      ...
                      className="flex-1 min-w-[140px] px-3.5 py-2 rounded-xl bg-primary text-slate-950 font-black text-xs hover:bg-primary/90 transition text-center shadow-md gold-glow flex items-center justify-center gap-1.5"
                    >
                      <span className="truncate">Date-by-Date Breakdown</span>
      ...
      {selectedAppClient && (
        <div className="space-y-6 animate-float-up">
          {/* Selected App Banner */}
          <div className="p-5 sm:p-6 rounded-3xl bg-white dark:bg-black/40 border border-slate-200 dark:border-white/10 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 min-w-0">
            <div className="flex items-center gap-3.5 min-w-0 flex-1">
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center text-slate-950 font-bold shrink-0 shadow-md overflow-hidden">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white flex flex-wrap items-center gap-2 truncate">
                  <span className="truncate">{selectedAppClient.company_name}</span>
                  <span className="text-xs font-semibold text-slate-400 truncate">({selectedAppClient.app_name || selectedAppClient.email})</span>
                </h2>
                <div className="flex flex-wrap items-center gap-3 mt-1.5 text-xs font-bold text-slate-500 dark:text-slate-300">
