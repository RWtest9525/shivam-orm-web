  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-4 py-8 bg-slate-50 dark:bg-base-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
      ...
      <div className="absolute top-4 right-4">
        <button
          onClick={toggleTheme}
          className="flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-1.5 text-xs font-bold text-slate-800 shadow-sm hover:bg-slate-100 dark:border-white/10 dark:bg-white/10 dark:text-slate-200 dark:hover:bg-white/20"
        >
      ...
      <div className="w-full max-w-md">
        ...
        <div className="glass-strong rounded-2xl p-6 shadow-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-base-900">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-10 pr-3 text-xs font-semibold text-slate-900 placeholder:text-slate-400 focus:border-accent-500 focus:outline-none dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-100"
                />
              </div>
            </div>

            <div>
              <label className="mb-1.5 block text-xs font-extrabold text-slate-800 dark:text-slate-200">Password</label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-10 pr-10 text-xs font-semibold text-slate-900 placeholder:text-slate-400 focus:border-accent-500 focus:outline-none dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-100"
                />
                <button
                  type="button"
          ...
          <div className="mt-6 border-t border-slate-200 pt-4 dark:border-white/10">
            <p className="mb-2.5 text-[10px] font-extrabold uppercase tracking-wider text-slate-600 dark:text-slate-400">1-Click Quick Demo Login</p>
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => fillDemo('client@dreamapps.com')}
                className="rounded-xl border border-accent-300 bg-accent-50 p-2.5 text-center text-xs font-bold text-accent-900 transition hover:bg-accent-100 dark:border-accent-500/30 dark:bg-accent-500/10 dark:text-accent-300"
              >
                <span className="block font-black text-accent-700 dark:text-accent-300">Client Panel</span>
                <span className="text-[10px] text-slate-600 dark:text-slate-400 font-semibold">DreamApps Ltd</span>
              </button>
              <button
                onClick={() => fillDemo('admin@shivamorm.com')}
                className="rounded-xl border border-emerald-300 bg-emerald-50 p-2.5 text-center text-xs font-bold text-emerald-900 transition hover:bg-emerald-100 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300"
              >
                <span className="block font-black text-emerald-700 dark:text-emerald-300">Super Admin</span>
                <span className="text-[10px] text-slate-600 dark:text-slate-400 font-semibold">Executive Control</span>
              </button>
            </div>
          </div>
        </div>

        <p className="mt-4 text-center text-[11px] text-slate-600 dark:text-slate-400 font-bold">
