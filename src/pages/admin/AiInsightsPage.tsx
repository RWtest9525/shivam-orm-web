...
import { Sparkles, Wand2, TrendingUp, TrendingDown, Minus, Loader2, AlertTriangle, Target, Lightbulb, ShieldCheck } from 'lucide-react';
...
export function AiInsightsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Top AI Banner */}
      <div className="bg-gradient-to-br from-black via-neutral-950 to-black border border-primary/25 rounded-2xl p-6 gold-glow relative overflow-hidden">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight" style={{ fontFamily: "'Playfair Display', serif" }}>
                AI Insights <span className="text-xs text-muted-foreground uppercase tracking-widest ml-2">by Equinox Pulse</span>
              </h2>
              ...
              {WINDOWS.map((w) => (
                <button
                  ...
                  className={`text-xs px-3 py-1 rounded-lg transition font-medium ${
                    win === w.key ? 'bg-primary/15 text-primary border border-primary/30' : 'text-neutral-400 hover:text-neutral-200'
                  }`}
                >
                  {w.label}
                </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-black/40 border border-white/5 space-y-3">
          <div className="flex items-center gap-2 text-base font-semibold text-white">
            <AlertTriangle className="w-4 h-4 text-rose-400" /> Root Causes
          </div>
          <p className="text-xs text-muted-foreground">What is driving current sentiment changes?</p>

          <div className="space-y-2.5 pt-1">
            {rootCauses.map((r, i) => (
              <div key={i} className="p-3.5 rounded-xl bg-black/30 border border-white/5 hover:border-rose-500/25 transition">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded border uppercase ${
                    r.impact === 'high' ? 'bg-rose-500/15 text-rose-400 border-rose-500/30' : 'bg-amber-500/15 text-amber-400 border-amber-500/30'
                  }`}>
                    {r.impact} IMPACT
                  </span>
                  <span className="text-[10px] uppercase text-muted-foreground font-semibold">{r.affectedCategory.replace('_', ' ')}</span>
                </div>
                <div className="text-xs sm:text-sm font-semibold text-white">{r.theme}</div>
                <div className="text-xs text-neutral-400 mt-1 italic border-l-2 border-primary/30 pl-2">
                  "{r.evidence}"
                </div>
              </div>
            ))}
        ...
        <div className="p-5 rounded-2xl bg-black/40 border border-white/5 space-y-3">
          ...
          <p className="text-xs text-muted-foreground">Where is brand momentum heading?</p>
          ...
          <div className="space-y-2.5 pt-1">
            {trends.map((t, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-3 p-3.5 rounded-xl bg-black/30 border border-white/5">
                  <div className={`w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0 ${isUp ? 'text-emerald-400' : 'text-rose-400'}`}>
                  ...
                  <div className={`text-sm font-bold shrink-0 ${isUp ? 'text-emerald-400' : 'text-rose-400'}`}>
                    {t.magnitude}
                  </div>
                </div>
              );
        ...
        <div className="p-5 rounded-2xl bg-black/40 border border-white/5 space-y-3">
          ...
          <p className="text-xs text-muted-foreground">Untapped upside &amp; low-effort wins</p>

          <div className="space-y-2.5 pt-1">
            {opportunities.map((o, i) => (
              <div key={i} className="p-3.5 rounded-xl bg-black/30 border border-white/5">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[9px] font-bold px-2 py-0.5 rounded border bg-emerald-500/15 text-emerald-400 border-emerald-500/30 uppercase">
                    {o.impact} IMPACT
                  </span>
                  <span className="text-[9px] font-bold px-2 py-0.5 rounded border border-white/10 text-neutral-300 uppercase">
                    {o.effort} EFFORT
                  </span>
                </div>
                <div className="text-xs sm:text-sm font-semibold text-white">{o.title}</div>
                <div className="text-xs text-neutral-400 mt-1">{o.rationale}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-black/40 border border-primary/20 gold-glow space-y-3">
          <div className="flex items-center gap-2 text-base font-semibold text-white">
            <Target className="w-4 h-4 text-primary" /> Executive Recommendations
          </div>
          <p className="text-xs text-muted-foreground">Ranked · Owner-assigned · Actionable ETA</p>

          <div className="space-y-2.5 pt-1">
            {recommendations.map((r, i) => (
              <div key={i} className="p-3.5 rounded-xl bg-black/40 border border-white/5 hover:border-primary/25 transition">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[9px] font-bold px-2 py-0.5 rounded border border-rose-500/30 text-rose-400 uppercase">
                    {r.priority}
                  </span>
                  <span className="text-[9px] font-bold px-2 py-0.5 rounded border border-primary/25 text-primary bg-primary/5 uppercase">
                    {r.owner}
                  </span>
                  <span className="text-[10px] text-muted-foreground ml-auto">ETA {r.etaDays}d</span>
                </div>
                <div className="text-xs sm:text-sm font-medium text-white">{r.action}</div>
              </div>
            ))}
          </div>
        </div>
